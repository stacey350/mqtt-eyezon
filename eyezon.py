import socket, struct, threading, queue, datetime, time, sys, configparser,codecs
...
class Daemon(threading.Thread):
  ...
  def run(self):
    ...
    def requestZoneTimerDump():
      ...
      alarmSocket.send("^02,$".encode('utf-8'))
    ...
    def processAlarmLogin():
      ...
      alarmSocket.send(password.encode('utf-8'))
    ...
    def processZoneStateChange(data):
      ...
      for i in range(0, len(data), 4):
        zones = bin(int(codecs.encode(codecs.decode((data[i:i+4]),"hex")[::-1],"hex"), 16))[2:].zfill(16)[::-1]
        for zone in range(0, len(zones), 1):
          zonesBuffer.append({
    ...
    def processPartitionStateChange(data):
      ...
      for i in range(0, len(data), 4):
          partition = i/4
          status = partitionLabel(int(codecs.encode(codecs.decode((data[i:i+4]),"hex")[::-1],"hex"), 16))
    ...
    def processZoneTimerDump(data):
      ...
      timers = []
      for i in range(0, (len(data)), 4):
        timerDelta =  (int("FFFF",16) - int(codecs.encode(codecs.decode((data[i:i+4]),"hex")[::-1],"hex"), 16))*5
        timers.append({
          "zone": i/4,
    ...
    while 1:
      ...
      try:
        ...
        if not rawData :
          ...
          for data in rawData.decode().split("\n"):
            data = data.strip()
